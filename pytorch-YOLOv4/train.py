...
def train(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', ncols=50) as pbar:
            ...
            print("Creating evaluation model using Yolov4 class...")
            eval_model = Yolov4(yolov4conv137weight=None, n_classes=config.classes, inference=True)
            ...
            if save_cp:
                ...
                if current_map > best_map:
                    ...
                    best_model_path = os.path.join(config.checkpoints, 'Yolov4_best.pth')
                    if isinstance(model, torch.nn.DataParallel):
                        torch.save(model.module.state_dict(), best_model_path)
                    else:
                        torch.save(model.state_dict(), best_model_path)
                    logging.info(f'New best model saved with mAP: {best_map:.4f} at epoch {epoch + 1}!')
                    
                if (epoch + 1) % 10 == 0 or (epoch + 1) == epochs:
                    save_path = os.path.join(config.checkpoints, f'{save_prefix}{epoch + 1}.pth')
                    if isinstance(model, torch.nn.DataParallel):
                        # 修正筆誤: model.moduel -> model.module
...
def get_args(**kwargs):
    ...
    parser.add_argument(
        ...
        dest='iou_type')
    parser.add_argument(
        '-keep-checkpoint-max', type=int, default=200000000,
    ...
    # for k in args.keys():
    #     cfg[k] = args.get(k)
    args_provided = {k: v for k, v in args.items() if v is not None}
    cfg.update(args_provided)
